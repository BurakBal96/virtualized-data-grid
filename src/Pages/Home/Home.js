...
export const Home = (props) => {
  ...
  const onCustomCellClick = (index, data) =>
    alert(
      `Column ${index + 1} clicked, data: \n${JSON.stringify(data, null, 2)}`
    );
    ...
    <div className={styles.homePage}>
      {loading && (
        <ContainerLoader
          ...
          text={`Creating ${ITEM_COUNT} items for list, this might take some time`}
        />
      )}
...
export const Row = inject("TableStore")(
  observer((props) => {
    ...
    return (
      <GridRow {...props} style={{ [CSS_BACKGROUND_VARIABLE]: background }} />
    );
  })
);
